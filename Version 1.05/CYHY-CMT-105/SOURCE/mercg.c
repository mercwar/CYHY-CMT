...
char gLastNavNode[MAX_PATH] = {0};
HWND hMercgHistoryList;
/* ---------------------------------------------------------
   UI CONSTRUCTION
--------------------------------------------------------- */
...
// --- TOP BUTTON BAR -----------------------------------------------------
//
CreateWindowA("BUTTON", "Dir",
    WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
    10, 10, 60, 25,
    hWnd, (HMENU)ID_MERCG_CHOOSE_DIR, hInstLocal, NULL);

CreateWindowA("BUTTON", "Open",
    WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
    80, 10, 60, 25,
    hWnd, (HMENU)ID_MERCG_OPEN_AVIS, hInstLocal, NULL);

CreateWindowA("BUTTON", "Save",
    WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
    150, 10, 60, 25,
    hWnd, (HMENU)ID_MERCG_SAVE_AVIS, hInstLocal, NULL);

CreateWindowA("BUTTON", "Scan",
    WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
    220, 10, 60, 25,
    hWnd, (HMENU)ID_MERCG_SCAN_DIR, hInstLocal, NULL);


//
// --- TREE VIEW (LEFT COLUMN) --------------------------------------------
//
hMercgTree = CreateWindowExA(
    WS_EX_CLIENTEDGE,
    WC_TREEVIEWA, "",
    WS_CHILD | WS_VISIBLE |
    TVS_HASLINES | TVS_LINESATROOT | TVS_HASBUTTONS | TVS_SHOWSELALWAYS,
    10, 45, 250, 360,
    hWnd, (HMENU)IDR_TREE_POPUP, hInstLocal, NULL);


//
// --- EDITOR (RIGHT COLUMN) ----------------------------------------------
//
hMercgEdit = CreateWindowExA(
    WS_EX_CLIENTEDGE,
    "EDIT", "",
    WS_CHILD | WS_VISIBLE |
...
//
// --- HISTORY LIST VIEW (LEFT COLUMN, BELOW TREE) ------------------------
//
hMercgHistoryList = CreateWindowExA(
    WS_EX_CLIENTEDGE,
    WC_LISTVIEWA, "",
    WS_CHILD | WS_VISIBLE |
...
col.cx = 120;
col.pszText = "Node";
ListView_InsertColumn(hMercgHistoryList, 0, &col);

col.cx = 120;
col.pszText = "Timestamp";
ListView_InsertColumn(hMercgHistoryList, 1, &col);


//
// --- TERMINAL (RIGHT COLUMN, BELOW EDITOR) -------------------------------
//
hMercgTerminal = CreateWindowExA(
    WS_EX_CLIENTEDGE,
    "EDIT", "",
    WS_CHILD | WS_VISIBLE |
    WS_VSCROLL | ES_MULTILINE | ES_READONLY | ES_AUTOVSCROLL,
    270, 415, 500, 160,
    hWnd, (HMENU)IDC_TERMINAL, hInstLocal, NULL);
...
case ID_HISTORY_LOADFILE_ADD:
{
    // 1. LOAD FILE (uses TreeView selection → gets filename as above)
    SendMessageA(hWnd, WM_COMMAND, MAKEWPARAM(ID_TREE_POPUP_LOADFILE, 0), 0);

    // 2. ADD TO HISTORY (uses gLastNavNode, etc.)
    SendMessageA(hWnd, WM_COMMAND, MAKEWPARAM(ID_HISTORY_ADD, 0), 0);
...
case ID_HISTORY_ADD:
{
    if (strlen(gLastNavNode) == 0) {
        AVIS_ERR("HISTORY", "No navigation node selected.");
        break;
    }

    // Build history file path
    char historyPath[MAX_PATH];
    sprintf(historyPath, "%s\\project_history.avis", gMercgRoot);

    FILE* f = fopen(historyPath, "a");
    if (!f) {
        AVIS_ERR("HISTORY", "Cannot open project history file.");
        break;
    }
    ...
    // Notify user
    SetWindowTextA(hMercgPropLabel, "NAV >> Project History Updated.");

    // Insert into ListView if active
    if (hMercgHistoryList) {
        LVITEMA item = {0};
        item.mask = LVIF_TEXT;
        item.pszText = gLastNavNode;
        int idx = ListView_InsertItem(hMercgHistoryList, &item);
        ListView_SetItemText(hMercgHistoryList, idx, 1, ts);
    }

    AVIS_MSG("HISTORY", "Recorded: %s", gLastNavNode);
    break;
}
case ID_TREE_POPUP_LOADFILE: {
    ...
    // --- ALWAYS CLIMB TO THE FILE NODE (parent) ---
    HTREEITEM hFileNode = hSel;
    HTREEITEM hParent = TreeView_GetParent(hMercgTree, hSel);
    if (hParent) {
        // User clicked a metadata child → parent is the file node
        hFileNode = hParent;
    }

    // --- Extract file path from the FILE NODE ---
    TVITEMA tvi = {0};
    char szNodeText[256];
    tvi.hItem = hFileNode;
    ...
    if (!TreeView_GetItem(hMercgTree, &tvi)) break;
    if (tvi.lParam == 0) break;

    const char* fullPath = (const char*)tvi.lParam;

    // --- AVIS MODE: Load file exactly like OPEN button ---
    if (FILE_SURF_MODE == 1) {
        if (MERCG_LoadFile(fullPath)) {
            SetWindowTextA(hMercgEdit, gAvisBuffer);
            lstrcpyA(gMercgCurrentFile, fullPath);
            AVIS_MSG("NAV", "ARTIFACT SYNC: %s loaded.", szNodeText);
        }
    }

    // --- HWND MODE unchanged ---
    else if (FILE_SURF_MODE == 0) {
        ...
        if (IsWindow(hTarget)) {
            ...
            if (szContent) {
                ...
                if (SendMessageTimeoutA(hTarget, WM_GETTEXT, 65535, (LPARAM)szContent,
                    SMTO_ABORTIFHUNG | SMTO_NOTIMEOUTIFNOTHUNG, 1000, &dwResult))
                {
                    SetWindowTextA(hMercgEdit, szContent);
                    AVIS_MSG("NAV", "SYNC SUCCESS: %llu bytes from %s",
                             (unsigned long long)strlen(szContent), szNodeText);
                }
                free(szContent);
            }
        }
...
lstrcpyA(gLastNavNode, text);

wsprintfA(navLog,
    "NAV >> %s  |  [Send to Project History] .",
    text
);
