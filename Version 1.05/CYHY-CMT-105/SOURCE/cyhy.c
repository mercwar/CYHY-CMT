...
const char * CYHY_CMT_DIR= "D:\\msys64\\home\\Demonizer\\Sentinel\\CYHY-CMT";


extern const char MERCG_CLASS[];
extern void MERCG_ForceEnvironment(const char* root); 
...
void EnsureFactoryExists(const char* root) {
    ...
    // Create \factory directory if missing
    if (_access(path, 0) != 0) {
        _mkdir(path);
        AVIS_INFO("FIO", "Created missing directory: %s", path);
    }

    // Create a dummy acit.txt if missing to prevent the FAIL log
    strcat(path, "\\acit.txt");
    if (_access(path, 0) != 0) {
        FILE* f = fopen(path, "w");
        if (f) {
            fprintf(f, "AVIS:\n artifact: acit.txt\n role: factory-default\n+\nReady.");
            fclose(f);
        }
    }
}
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrev, LPSTR lpCmdLine, int nShowCmd)
{
    ...
    // 3. Setup Environment (Data Only)
    MERCG_ForceEnvironment(CYHY_CMT_DIR);

    // 4. Create Main Window FIRST
    // We create the window HIDDEN initially to prevent the Err: 5 flicker
    HWND hWnd = CreateWindowExA(0, CYHY_CLASS, "CYHY Project Editor [Logged]", 
        WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, 1000, 750, 
        NULL, NULL, hInst, NULL);

    if (!hWnd) {
        char err[64];
        sprintf(err, "FATAL: Main Window Failed. Err: %lu", GetLastError());
        MessageBoxA(NULL, err, "AVIS ENGINE", MB_ICONERROR);
        return 0;
    }

    // 5. Initialize MERCG Subsystem (Now that hWnd exists)
    AVIS_INFO("CORE", "Main UI Handle: 0x%p", (void*)hWnd);
    MERCG_Init(hInst); 
    ...
    AVIS_INFO("CORE", "CYHY [ROBOT] [MERC-G] READY...");
...
LRESULT CALLBACK CyhyWndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        ...
        case ID_OPTIONS_TOPMOST: {
            ...
            // UI Feedback: Check/Uncheck the menu item
            CheckMenuItem(GetMenu(hWnd), ID_OPTIONS_TOPMOST, isTop ? MF_UNCHECKED : MF_CHECKED);
            AVIS_INFO("CORE", "Main Editor Z-Order Shift: %s", isTop ? "NORMAL" : "TOPMOST");
            break;
        }
